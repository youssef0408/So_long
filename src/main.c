...
/*   Updated: 2023/09/28 18:37:47 by yothmani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (map->map_table[y][x] == '1')
	{
		if (mlx_image_to_window(info->mlx, info->img_wall, x * SIZE_IMG, y * SIZE_IMG ) < 0)
			error();	
	}
	if (map->map_table[y][x] == 'P')
	{
		if (mlx_image_to_window(info->mlx, info->img_player, x * SIZE_IMG, y * SIZE_IMG ) < 0)
			error();	
	}
}
...
void ft_render_winwow(t_map *map)
{
	...
	if (!(info.mlx = mlx_init((map->width) * SIZE_IMG, map->height * SIZE_IMG , "MLX42", false)))
	{
...
int	show_command(void)
{
	...
	scanf("%c", &c);
	command = c;
	if (command != 'w' && command != 'W' && command != 'A'
		&& command != 'a' && command != 's' && command != 'S'
		&& command != 'D' && command != 'd' && command != 27
		&& command != 113 && command != 10)
	{
...
int	main(void)
{
	...
	mat.map_table = ft_create_render_map(&mat, fd);
	close(fd);
